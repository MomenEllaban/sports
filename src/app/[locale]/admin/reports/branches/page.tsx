...
export default async function BranchesReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <><div className="border-b border-slate-800 pb-4"><h1 className="text-2xl font-black text-slate-100">أداء الفروع والمبيعات</h1><p className="text-xs text-slate-400">مقارنة إيراد الفروع وعدد الفواتير خلال الفترة.</p></div><ReportTableClient type="branches" rows={page.result.rows} total={page.result.total} page={page.result.page} pageSize={page.result.pageSize} totalPages={page.result.totalPages} branches={page.branches} filters={page.filters} /></>;
}
