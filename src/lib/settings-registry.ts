...
export const SETUP_GROUPS = [
  ...
  { id: 'returns', ar: 'المرتجعات والاستبدال', en: 'Returns & exchange' },
...
export const SETTINGS_REGISTRY: SettingDef[] = [
  ...
  { key: 'discount.stacking', group: 'loyalty', groupAr: 'الولاء والخصومات', labelAr: 'قواعد الجمع بين الخصومات', labelEn: 'Discount stacking rules', helpAr: 'هل يجتمع الكوبون مع الولاء؟ ومع خصم المدير؟ (T16).', helpEn: 'May coupons combine with loyalty / manager PIN? (T16).', type: 'list', required: false, sensitive: false, featureGate: 'discount-pipeline', owner: 'CLIENT', defaultValue: { allowCouponLoyalty: true, allowCouponPin: false } },
  // ── Stock ──────────────────────────────────────────────
  ...
  { key: 'sizecharts', group: 'store', groupAr: 'بيانات المتجر والفروع', labelAr: 'جداول المقاسات', labelEn: 'Size charts', helpAr: 'جداول مقاسات لكل قسم، تظهر في صفحة المنتج (T07).', helpEn: 'Per-category size charts shown on product pages (T07).', type: 'list', required: false, sensitive: false, featureGate: 'storefront', owner: 'CLIENT', defaultValue: [] },
  { key: 'reports.deadStockDays', group: 'stock', groupAr: 'المخزون', labelAr: 'مدة الركود للأصناف الميتة (أيام)', labelEn: 'Dead-stock window (days)', helpAr: 'صنف بمخزون بلا بيع خلالها = ميت (T12). الافتراضي 60.', helpEn: 'Stocked but unsold within = dead (T12). Default 60.', type: 'number', required: false, sensitive: false, featureGate: 'reports', owner: 'CLIENT', defaultValue: 60 },
  // T-RMA returns policy (all DEFAULT_UNCONFIRMED until the client confirms).
  { key: 'returns.enabled', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'تفعيل المرتجعات', labelEn: 'Enable returns', helpAr: 'إتاحة طلبات المرتجع/الاستبدال في كل القنوات.', helpEn: 'Allow return/exchange requests on all channels.', type: 'boolean', required: true, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: true },
  { key: 'returns.windowDays', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'مدة الاسترجاع (أيام)', labelEn: 'Return window (days)', helpAr: 'خلالها يُقبل المرتجع من تاريخ التسليم/البيع.', helpEn: 'Returns accepted within this long after delivery/sale.', type: 'number', required: true, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: 14 },
  { key: 'returns.exchangeWindowDays', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'مدة الاستبدال (أيام)', labelEn: 'Exchange window (days)', helpAr: 'لتغيير المقاس عادة. اتركها فارغة = نفس مدة الاسترجاع.', helpEn: 'For size swaps usually. Empty = same as return window.', type: 'number', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: 14 },
  { key: 'returns.nonReturnableCategories', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'أقسام غير قابلة للاسترجاع', labelEn: 'Non-returnable categories', helpAr: 'slugs الأقسام المستثناة (مثل ملابس السباحة والدعامات لأسباب صحية) — قرارك.', helpEn: 'Excluded category slugs (e.g. swimwear, supports) — your call.', type: 'list', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: ['swimming-gear', 'medical-protection'] },
  { key: 'returns.requireReceipt', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'الإيصال إجباري في POS؟', labelEn: 'Receipt required at POS?', helpAr: 'بدونه: البحث برقم الهاتف + موافقة مدير بـ PIN.', helpEn: 'Without it: phone lookup + manager PIN approval.', type: 'boolean', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: true },
  { key: 'returns.reasons', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'أكواد الأسباب', labelEn: 'Reason codes', helpAr: 'القائمة المعتمدة للأسباب (SIZE_ISSUE/DEFECTIVE/WRONG_ITEM/NOT_AS_DESCRIBED/CHANGED_MIND/OTHER).', helpEn: 'Approved reason list.', type: 'list', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: ['SIZE_ISSUE', 'DEFECTIVE', 'WRONG_ITEM', 'NOT_AS_DESCRIBED', 'CHANGED_MIND', 'OTHER'] },
  { key: 'returns.requirePhotoForReasons', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'أسباب تتطلب صورة', labelEn: 'Reasons requiring photo', helpAr: 'مثل DEFECTIVE — صورة إثبات إجبارية عند الطلب.', helpEn: 'E.g. DEFECTIVE — proof photo mandatory.', type: 'list', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: ['DEFECTIVE'] },
  { key: 'returns.refundDeliveryFee', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'سياسة رد التوصيل', labelEn: 'Delivery refund policy', helpAr: 'ALWAYS أو FULL_RETURN_OR_OUR_FAULT أو NEVER — راجعها مع المحاسب.', helpEn: 'ALWAYS, FULL_RETURN_OR_OUR_FAULT or NEVER — confirm with accountant.', type: 'select', required: true, sensitive: false, featureGate: 'returns', owner: 'ACCOUNTANT', options: ['ALWAYS', 'FULL_RETURN_OR_OUR_FAULT', 'NEVER'], defaultValue: 'FULL_RETURN_OR_OUR_FAULT' },
  { key: 'returns.restockingFeePct', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'رسوم إعادة التخزين %', labelEn: 'Restocking fee %', helpAr: 'تُخصم فقط لسبب CHANGED_MIND عند التفعيل (0 = معطلة).', helpEn: 'Deducted only for CHANGED_MIND when enabled (0 = off).', type: 'number', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: 0 },
  { key: 'returns.whoPaysReturnShipping', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'من يدفع شحن الإرجاع؟', labelEn: 'Who pays return shipping?', helpAr: 'CUSTOMER أو STORE أو STORE_IF_OUR_FAULT.', helpEn: 'CUSTOMER, STORE or STORE_IF_OUR_FAULT.', type: 'select', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', options: ['CUSTOMER', 'STORE', 'STORE_IF_OUR_FAULT'], defaultValue: 'STORE_IF_OUR_FAULT' },
  { key: 'returns.cashRefundManagerThreshold', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'حد الاسترداد النقدي لموافقة المدير (ج.م)', labelEn: 'Cash refund manager threshold (EGP)', helpAr: 'فوقه يلزم PIN مدير في POS.', helpEn: 'Above it a manager PIN is required at POS.', type: 'number', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: 500 },
  { key: 'returns.autoApproveMaxValue', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'سقف الاعتماد التلقائي في POS (ج.م)', labelEn: 'POS auto-approve cap (EGP)', helpAr: 'مرتجع المحل داخله يُعتمد تلقائياً، وفوقه بـ PIN.', helpEn: 'In-store returns within auto-approve, above needs PIN.', type: 'number', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: 1000 },
  { key: 'returns.maxReturnsPerCustomerPerMonth', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'حد مرتجعات العميل شهرياً (تحذير)', labelEn: 'Max returns per customer/month (warn)', helpAr: 'تجاوزه تنبيه فقط لا منع.', helpEn: 'Exceeded = warning only, not a block.', type: 'number', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: 5 },
  { key: 'returns.allowedRefundMethods', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'طرق الاسترداد المسموحة', labelEn: 'Allowed refund methods', helpAr: 'CASH/ORIGINAL_GATEWAY/INSTAPAY/VODAFONE/BANK_TRANSFER.', helpEn: 'Subset of payout methods.', type: 'list', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: ['CASH', 'ORIGINAL_GATEWAY', 'INSTAPAY', 'VODAFONE', 'BANK_TRANSFER'] },
  { key: 'returns.slaHours', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'مدة المعالجة المعلنة (ساعات)', labelEn: 'Promised processing time (hours)', helpAr: 'تظهر للعميل في صفحة الحالة.', helpEn: 'Shown to the customer on the status page.', type: 'number', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: 72 },
  { key: 'returns.policyText', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'نص السياسة (عربي/إنجليزي)', labelEn: 'Policy text (AR/EN)', helpAr: 'يظهر في المتجر والإيصال. اكتبه بصياغتك النهائية.', helpEn: 'Shown in store and receipts. Your final wording.', type: 'list', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: [{ ar: 'الاستبدال والاسترجاع خلال 14 يوماً بالفاتورة', en: 'Returns within 14 days with receipt' }] },
  { key: 'returns.receiveBranchDefault', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'الاستلام الافتراضي', labelEn: 'Default receive branch', helpAr: 'SALE_BRANCH (فرع البيع) أو معرف فرع ثابت.', helpEn: 'SALE_BRANCH or a fixed branch id.', type: 'text', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: 'SALE_BRANCH' },
  { key: 'returns.reverseCourierEnabled', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'شحن الإرجاع العكسي', labelEn: 'Reverse courier pickup', helpAr: 'يتطلب إعداد Bosta reverse — وإلا الاستلام في الفرع فقط.', helpEn: 'Needs Bosta reverse setup — else in-branch receive only.', type: 'boolean', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: false },
  { key: 'loyalty.allowNegativeOnReturn', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'السماح بسالب الولاء عند المرتجع', labelEn: 'Allow negative loyalty on return', helpAr: 'الافتراضي: سالب صغير مسموح مع منع الاستبدال حتى التعويض.', helpEn: 'Default: small negative allowed, redeem blocked until covered.', type: 'boolean', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: true },
  { key: 'coupons.restoreOnFullReturn', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'استعادة الكوبون عند الإرجاع الكامل', labelEn: 'Restore coupon on full return', helpAr: 'الافتراضي: لا تُستعاد الاستخدامات.', helpEn: 'Default: usages are not restored.', type: 'boolean', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: false },
  { key: 'returns.notifyWhatsapp', group: 'returns', groupAr: 'المرتجعات والاستبدال', labelAr: 'إشعار واتساب لحالات المرتجع', labelEn: 'WhatsApp notifications for returns', helpAr: 'يعمل فقط لو تكامل واتساب مفعّل.', helpEn: 'Works only if WhatsApp is enabled.', type: 'boolean', required: false, sensitive: false, featureGate: 'returns', owner: 'CLIENT', defaultValue: true },
  // ── ETA ────────────────────────────────────────────────
  ...
  { key: 'eta.branchCode', group: 'eta', groupAr: 'الفاتورة الإلكترونية ETA', labelAr: 'كود الفرع لدى ETA', labelEn: 'ETA branch code', helpAr: 'كود الفرع المسجل في المنظومة (من المحاسب). يُستخدم في T14.', helpEn: 'Branch code in ETA (from accountant). Used in T14.', type: 'text', required: false, sensitive: false, featureGate: 'eta', owner: 'ACCOUNTANT', example: '0', defaultValue: '0' },
  { key: 'eta.activityCode', group: 'eta', groupAr: 'الفاتورة الإلكترونية ETA', labelAr: 'كود النشاط', labelEn: 'Activity code', helpAr: 'كود نشاط بيع التجزئة الرياضية (من المحاسب). يُستخدم في T14.', helpEn: 'Retail activity code (from accountant). Used in T14.', type: 'text', required: false, sensitive: false, featureGate: 'eta', owner: 'ACCOUNTANT', example: '5239', defaultValue: '' },
  { key: 'eta.signingUrl', group: 'eta', groupAr: 'الفاتورة الإلكترونية ETA', labelAr: 'رابط خدمة التوقيع الإلكتروني', labelEn: 'E-signature service URL', helpAr: 'رابط خدمة توقيع المستندات CAdES (HSM/Token). إلزامي في production (T14).', helpEn: 'CAdES signing service URL. Mandatory in production (T14).', type: 'secret', required: false, sensitive: true, featureGate: 'eta', owner: 'ACCOUNTANT', defaultValue: '' },
  // ── WhatsApp ───────────────────────────────────────────
