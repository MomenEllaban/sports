...
import { DollarSign, ShieldCheck, FileCheck, CheckCircle2, TrendingUp, AlertTriangle } from 'lucide-react';

export const revalidate = 10;

export default async function AdminAccountingPage() {
  // Fetch overall financial aggregates
        ...
        <main className="p-6 space-y-6 overflow-y-auto">
          <div className="flex justify-between items-center border-b border-slate-800 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                متابعة الأرباح والخسائر، ضريبة القيمة المضافة 14%، وإصدارات الإيصالات الإلكترونية
              </p>
            </div>
            <span className="px-3 py-1.5 rounded-full bg-emerald-500/20 text-emerald-400 font-bold text-xs border border-emerald-500/30">
              صلاحية الحسابات والمدير العام فقط
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-2">
              ...
              <div className="text-[11px] text-slate-400">إيجارات، كهرباء، صيانة</div>
            </div>

            <div className="glass-panel p-5 rounded-2xl border border-slate-800 space-y-2">
              <div className="text-xs text-slate-400">تسويات الدفع عند الاستلام (COD)</div>
              <div className="text-2xl font-black text-emerald-400">100% متطابقة</div>
              <div className="text-[11px] text-emerald-400">مبالغ المندوبين محصلة وموردة</div>
            </div>
          </div>
          ...
          <div className="glass-panel p-6 rounded-3xl border border-slate-800 space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              ...
              <span className="text-xs text-slate-400">مفعل في الوضع التلقائي</span>
            </div>
                ...
                <tbody className="divide-y divide-slate-800">
                  {taxInvoices.map((tax) => (
                    <tr key={tax.id} className="hover:bg-slate-900/50">
                      ...
                      <td className="p-3">
                        <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-400 font-bold text-[10px] border border-emerald-500/30 flex items-center gap-1 w-fit">
                          <CheckCircle2 className="w-3 h-3" /> {tax.status}
                        </span>
                      </td>
