...
export default async function CatalogPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: SearchParams }) {
  ...
  return <main className="flex-1 max-w-7xl mx-auto px-4 py-8 space-y-8 w-full">
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800 pb-6"><div><h1 className="text-3xl font-black text-slate-100">{isAr ? 'كتالوج الأجهزة والمعدات الرياضية' : 'Sports equipment catalog'}</h1><p className="text-xs text-slate-400 mt-1">{isAr ? `تصفح التشكيلة الكاملة (${total.toLocaleString()} صنف)` : `Browse the full selection (${total.toLocaleString()} items)`}</p></div><form method="GET" className="relative w-full md:w-80" role="search"><label htmlFor="catalog-search" className="sr-only">{isAr ? 'بحث في الكتالوج' : 'Search the catalog'}</label><input id="catalog-search" type="search" name="query" defaultValue={sp.query || ''} placeholder={isAr ? 'بحث بالاسم أو SKU أو الباركود' : 'Search by name, SKU, or barcode'} className="w-full pl-4 pr-10 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs text-slate-200" /><input type="hidden" name="category" value={sp.category || ''} /><input type="hidden" name="brand" value={sp.brand || ''} /><input type="hidden" name="sort" value={sort} /><button type="submit" aria-label={isAr ? 'بحث' : 'Search'} className="absolute right-3 top-2.5 text-slate-400"><Search className="w-4 h-4" /></button></form></div>
    <div className="flex flex-wrap items-center gap-2"><div className="app-scrollbar app-scrollbar-horizontal flex gap-2 overflow-x-auto pb-2 scrollbar-none"><Link href={href({ category: undefined, page: undefined })} className={`min-h-[44px] inline-flex items-center px-4 rounded-xl text-xs font-bold ${!sp.category ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-400'}`}>{isAr ? 'كل التصنيفات' : 'All categories'}</Link>{categories.map((cat) => <Link key={cat.id} href={href({ category: cat.slug, page: undefined })} className={`min-h-[44px] inline-flex items-center px-4 rounded-xl text-xs font-bold whitespace-nowrap ${sp.category === cat.slug ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-400'}`}>{isAr ? cat.nameAr : cat.nameEn}</Link>)}</div><CatalogSortSelect value={sort} isAr={isAr} /></div>
    {brands.length > 0 && <div className="app-scrollbar app-scrollbar-horizontal flex gap-2 overflow-x-auto pb-2"><Link href={href({ brand: undefined, page: undefined })} className={`min-h-[44px] inline-flex items-center rounded-full border px-3 text-xs ${!sp.brand ? 'status-info' : 'border-slate-700 text-slate-400'}`}>{isAr ? 'كل الماركات' : 'All brands'}</Link>{brands.map((brand) => <Link key={brand.id} href={href({ brand: brand.slug, page: undefined })} className={`min-h-[44px] inline-flex items-center rounded-full border px-3 text-xs ${sp.brand === brand.slug ? 'status-info' : 'border-slate-700 text-slate-400'}`}>{isAr ? brand.nameAr : brand.nameEn}</Link>)}</div>}
