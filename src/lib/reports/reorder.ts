...
export async function getReorderRows(query: { q?: string; branchId?: string; page: number; pageSize: number; requested?: 'requested' | 'unrequested' }) {
  ...
  const rows = await prisma.branchInventory.findMany({
    ...
    include: { branch: { select: { id: true, name: true } }, product: { select: { id: true, nameAr: true, nameEn: true, sku: true, barcode: true, price: true, costPrice: true, category: { select: { nameAr: true } }, brand: { select: { nameAr: true } } } }, reorderRequests: { where: { status: 'REQUESTED' }, orderBy: { requestedAt: 'desc' }, take: 1 } },
