...
export default async function AdminProductsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <><div><h1 className="text-2xl font-black text-slate-100">كتالوج المنتجات والمعدات</h1><p className="text-xs text-slate-400 mt-0.5">إدارة الأصناف والأسعار مع pagination وفلاتر URL.</p></div><div className="glass-panel rounded-3xl border border-slate-800 overflow-hidden p-4 animate-fade-up"><ProductsManager products={products.map((p) => ({ ...p, price: num(p.price), costPrice: num(p.costPrice) }))} categories={categories} brands={brands} serverSide totalCount={total} initialPage={page} initialSearch={sp.query || ''} initialCategoryId={sp.categoryId || ''} initialBrandId={sp.brandId || ''} initialStatus={sp.status || ''} /></div></>;
}
