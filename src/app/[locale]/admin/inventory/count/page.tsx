...
export default async function StocktakePage() {
  ...
  const [branches, products, sessions] = await Promise.all([
    ...
    prisma.stocktakeSession.findMany({ where: allowed === null ? {} : { branchId: { in: allowed } }, include: { branch: { select: { id: true, name: true } }, _count: { select: { lines: true } } }, orderBy: { createdAt: 'desc' }, take: 30 }),
  ]);
  return <><div className="border-b border-slate-800 pb-4"><h1 className="text-2xl font-black text-slate-100">الجرد وتسوية المخزون</h1><p className="text-xs text-slate-400">جلسة batch لكل أصناف الفرع: حفظ مسودة، مراجعة الفروقات، ثم اعتماد ذري مع سجل InventoryLog.</p></div><StocktakeClient branches={branches} products={products} sessions={sessions.map((item) => ({ ...item, createdAt: item.createdAt.toISOString(), startedAt: item.startedAt.toISOString(), approvedAt: item.approvedAt?.toISOString() || null }))} /></>;
}
