...
export default async function AdminReportsPage() {
  ...
  return <>
    <div className="border-b border-slate-800 pb-4"><h1 className="text-2xl font-black text-slate-100 flex items-center gap-2"><BarChart3 className="w-6 h-6 text-blue-400" />{isAr ? 'التقارير التحليلية' : 'Analytics reports'}</h1><p className="text-xs text-slate-400 mt-0.5">{isAr ? 'فهرس التقارير الحقيقية؛ كل تقرير له route وفلاتر وتصدير مستقل.' : 'Real report index; every report has its own route, filters, and export.'}</p></div>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{reports.map((report) => { const Icon = report.icon; return <Link key={report.href} href={report.href} className="group rounded-3xl border border-slate-800 bg-slate-900/60 p-5 transition hover:border-blue-500/50"><div className="flex items-start gap-3"><span className="rounded-xl bg-blue-500/10 p-3 text-blue-300"><Icon className="w-5 h-5" /></span><div className="min-w-0 flex-1"><h2 className="font-extrabold text-slate-100">{isAr ? report.labelAr : report.labelEn}</h2><p className="mt-1 text-xs leading-5 text-slate-400">{isAr ? report.descriptionAr : report.descriptionEn}</p></div><ArrowLeft className="w-4 h-4 text-slate-600 group-hover:text-blue-300" /></div></Link>; })}</div>
  </>;
}
