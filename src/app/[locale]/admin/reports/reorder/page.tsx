...
export default async function ReorderReportPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [result, branches, suppliers] = await Promise.all([
    ...
    prisma.branch.findMany({ where: { isActive: true, ...(allowed === null ? {} : { id: { in: allowed } }) }, select: { id: true, name: true }, orderBy: { name: 'asc' } }),
    prisma.supplier.findMany({ select: { id: true, name: true }, orderBy: { name: 'asc' } }),
  ]);
  return <><div className="border-b border-slate-800 pb-4"><h1 className="text-2xl font-black text-slate-100">كشكول النواقص</h1><p className="text-xs text-slate-400">أصناف تحت حد إعادة الطلب، الكمية المقترحة، ومتابعة من طلبها ومتى.</p></div><ReorderClient rows={result.rows} total={result.total} page={result.page} totalPages={result.totalPages} branches={branches} suppliers={suppliers} filters={{ q: value('q') || '', branch: requestedBranch || '', requested: value('requested') || '' }} /></>;
}
